...
std::string pose_frame_id("/aft_mapped");
...
bool first = true;
double position_change_thresh = 0.0;
double orientation_change_thresh = 0.0;
bool updated = false;
...
// void publish_project_2d_map(const la3dm::BGKLOctoMap &map) {
...
void cloud_callback(const sensor_msgs::PointCloud2ConstPtr &cloud) {
    // ROS_INFO_STREAM("GOT CLOUD");
    tf::StampedTransform transform;
    try {
        listener->lookupTransform(frame_id, pose_frame_id, cloud->header.stamp, transform);
...
int main(int argc, char **argv) {
    ...
    nh.param<std::string>("pose_frame_id", pose_frame_id, pose_frame_id);
    ...
    ROS_INFO_STREAM("Parameters:" << std::endl <<
                    ...
                    "pose_frame_id: " << pose_frame_id << std::endl <<
    ...
    ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>(cloud_topic, 5, cloud_callback);
